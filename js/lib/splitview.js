(function( $ ){
	function internalInit(container) {
		...
		$(window).bind('resize.SplitView', checkResolution);
	...
	var methods = {
		...
		destroy : function( ) {
			return this.each(function(){
				$(window).unbind('.SplitView');this.id
			})
		},
	...
	$.fn.SplitView = function( method ) {
		if ( methods[method] ) {
			return methods[method].apply( this, Array.prototype.slice.call( arguments, 1 ));
		} else if ( typeof method === 'object' || ! method ) {
			return methods.init.apply( this, arguments );
		} else {
			$.error( 'Method ' +  method + ' does not exist on jQuery.SplitView' );
		}    
	};
})( jQuery );
